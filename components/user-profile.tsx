...
import { BoxMemberProject } from "./box-project_user"
...
export function UserProfile({ node, ...props }: UserProfileProps) {
  ...
  const { data: group_project, error: group_projectError } = useSWR(() => 'https://fed.septembre.io/jsonapi/group/federage', fetcher)
  ...
  return (
    <>
      ...
      {
        group_project?.data
          .filter((project_member) => project_member.relationships.uid.data.id.includes(node.id))
          .map((project_member) => (
            <div key={project_member.id} className={openTab === 2 ? "block" : "hidden"}>
              <BoxMemberProject key={project_member.id} project_member={project_member} useringroup={useringroup} />
            </div>
          ))
      }
    </>
