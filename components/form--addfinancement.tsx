...
interface FormAddfinancementProps extends React.HTMLProps<HTMLFormElement> {}
...
export function FormAddfinancement({ className, categorieprj, assogroup, ...props }: FormAddfinancementProps) {
  ...
  const onSubmit = async (event) => {
    ...
    const response = await fetch("/api/evolutionfin", {
      method: "POST",
      body: data,
    })
  ...
  {t("Type d’apport")} <span className="text-sm text-red-500">*</span>
          ...
          <div className="grid gap-2">
                <label className="font-semibold text-text" htmlFor="grid-state">
      {t("quel groupe?")} <span className="text-sm text-red-500">*</span>
       </label>
