...
export function BoxMescomptes({   groupes, sousgroupes, membres, memberships, membershipssgroup, parente, userid, ttssgroupes, financementssg, user }: BoxMescomptesProps) {
  ...
  if (!stringtest) return <div>Loading stringtest ...</div>
	return (
		...
		{memberships && memberships
			.filter((userin) => userin.uid.id.includes(userid))
			.slice(0, 1)
...
<br/>
	...
	{groupsmemberships?.length ? (
	  ...
	  {groupsmemberships
	    ...
	    .filter((userin) => userin.uuid_2.includes(userid)&& userin.id.includes(groupssubgroups.subgroup_tree))
