...
export default function AlluserlistPage({ menus, blocks, users, allfederages, allfinancements, }: AlluserlistPageProps) {
  ...
  const [datafinancement, setData] = useState(null)
    const [isLoading, setLoading] = useState(false)

    useEffect(() => {
      setLoading(true)
      fetch('https://fed.septembre.io/jsonapi/node/financement/011a3b3f-c574-48ee-873a-daa25723c2ac?resourceVersion=id%3A40')
        .then((res) => res.json())
        .then((datafinancement) => {
          setData(datafinancement)
          setLoading(false)
        })
    }, [])

    if (isLoading) return <p>Loading...</p>
    if (!datafinancement) return <p>No profile data</p>
...
Use effect
<div>
      <h1>{datafinancement.data.id}</h1>
      <p>{datafinancement.data.attributes.title}</p>
    </div>
    </p>
...
<p>Id du node financement : {onefinancement.path}</p>
         </li>
       ))}
    </Layout>
  )
