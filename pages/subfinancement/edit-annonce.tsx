...
import { FormAnnonceEdit } from "components/form--annonce-edit"
...
{groupeid.id}
      ...
      {fin
      .map((fintoedit) => (
        <span key={fintoedit.id}>

        <FormAnnonceEdit className="max-w-2xl mx-auto mt-10 p-2" key={gid} categorieprj={categorieprj}  node={fintoedit} groupe={query.query}/>
...
const querygroupe = context.query.query
  ...
  const fin = await drupal.getResourceCollectionFromContext<DrupalNode[]>(
    ...
    context,
    {
      params: new DrupalJsonApiParams()
        .addInclude(["field_document_s_annexe_s_.field_media_document", "uid.user_picture", "field_choisir_une_categorie"])
  ...
  const groupe = await drupal.getResourceCollectionFromContext<DrupalTaxonomyTerm>(
    "group--federage",
    context,
    {
       params: new DrupalJsonApiParams()
